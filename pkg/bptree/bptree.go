...
func nodeAppendRange(
	...
	if srcOld+n > old.nkeys() || dstNew+n > new.nkeys() {
		panic("srcOld+n > old.nkeys() || dstNew+n > new.nkeys()")
	}
...
func (tree *BTree) Delete(key []byte) bool {
	...
	if len(key) > BTREE_MAX_KEY_SIZE {
		panic("Key size exceeds maximum allowed")
	}
	if tree.root == 0 {
		return false
	}
...
func (tree *BTree) Insert(key []byte, val []byte) {
	...
	if len(key) > BTREE_MAX_KEY_SIZE {
		panic("Key size exceeds maximum allowed size")
	}
	if len(val) > BTREE_MAX_VAL_SIZE {
		panic("Value size exceeds maximum allowed size")
	}
	if tree.root == 0 {
